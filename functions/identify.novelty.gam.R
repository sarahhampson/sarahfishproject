# The function developed by Pandolfi et al. (2020) 
# modified to include Gower dissimilarity matrices
...
identify.novel.gam<-function(site.sp.mat, alpha, 
  ...
  #site.sp.mat <- functional_matrix_list[[119]]
  ...
  if(!metric %in% c("chord", "SQchord", "hellinger", "Gower")){
  ...
  if(metric == "Gower"){
    ...
    # Scale dissimilarities differently to avoid y-axis scales close to 1
    site.dist <- (site.dist - min(site.dist)) / (max(site.dist) - min(site.dist))
  }
