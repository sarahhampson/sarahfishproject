# A function which modifies identify.novel.gam (Pandolfi et al. 2021) 
# in which the dissimilarity matrix is already provided

func.identify.novel.gam<- function(site.dist.mat, alpha, 
                             site, plot=TRUE, plot.data=FALSE,
                             gam.max.k = -1){
  
  # Verification variables
  #site.dist.mat <- functionaldissims_dist[[5]]
  #site.sp.mat <- seasonal_matrix_list[[5]]
  #site <- "TRIAL"
  #alpha <- 0.05
  #gam.max.k = -1
  ...
  #### Need to change this part so it runs youngest to oldest! ####
  ...
  #### NOT USING THIS PORTION AS THE DISSIMS MATRIX ALREADY EXISTS ####
  ...
  #### USING THIS BIT BELOW ####
  
  # Not sure if I need this bit?
  # convert bin names to numbers to use as continuous variable
  bins <- as.numeric(rownames(site.dist.mat))
  
  # Convert site dist into a dataframe matrix
  # Convert site.dist.mat into df
  site.dist.mat <- as.matrix(site.dist.mat)
  ...
  set.k <- ifelse(dim(site.dist.mat)[1] > 10,
                  gam.max.k,
                  ifelse(dim(site.dist.mat)[1] > 4,
                         floor(dim(site.dist.mat)[1])/2,
                         NA))
  ...
  # time t and t-1
  seq.dist <- c(NA, diag(site.dist.mat[-1, -dim(site.dist.mat)[2]]))
  ...
  # earliest time point)
  min.dist <- sapply(1:dim(site.dist.mat)[1], function(n){
    if(n==1){return(NA)}
    min(site.dist.mat[n,1:n][-n])
  })
  ...
  return.data <- data.frame(site = site,
                            bins = rownames(site.dist.mat),
                            bin.lag = c(NA, abs(diff(as.numeric(rownames(site.dist.mat))))),
                            seq.dist = seq.dist,
